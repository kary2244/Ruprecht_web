...
const FloresArreglos = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        // Obtener solo productos con type_candle = 5 para flores y arreglos
        const response = await axios.get(`${API_BASE_URL}/candles`, {
          params: { type_candle: 5, limit: 100 }
        })
        const candlesData = response.data.data || response.data
        const transformedProducts = candlesData.map((candle) => ({
          id: candle.id,
          name: candle.nombre,
          description: `Arreglo floral ${candle.medidas}`,
          price: candle.costo,
          category: 'Flores y Arreglos',
          size: candle.medidas,
          weight: candle.peso,
  ...
  return (
    <div className="products-page">
      <div className="products-header">
        <h1 className="products-title">FLORES Y ARREGLOS</h1>
        <p className="products-subtitle">
          Arreglos florales y productos decorativos únicos hechos a mano
        </p>
      </div>
